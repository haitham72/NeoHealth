...
official DHA/DoH/MOHAP regulations plus 3 on-topic academic papers by one of the
interviewing firm's own co-founders sit in the same index, but the research papers are
never dressed up with regulation styling (§6.3). **On corpus size: quote
`GET /corpus-stats`, never a number from a local file.** The funnel is
68 PDFs downloaded per `corpus_urls.txt` → 40 parsed automatically into
`parsed_documents.json` (37 official + 3 research) → 28 written to `needs_manual.json`
instead, because their templates carry no printed `doc_code`/`version`/`effective_date`
footer for `ingest.py` to read and it correctly declined to guess (§2.2) → 27 of
those 28 OCR-extracted by hand, 24 recovered and loaded + 3 already-parsed official
documents restored (they were missing from production, not from
`parsed_documents.json`) = **64 documents in the corpus** (§2.2a). 4 records were
ultimately dropped as unrecoverable or content-free, with reasons recorded in
`ingestion/ocr_extraction.json`. Generation can run against OpenAI
...
a human to finish by hand (`backend/ingestion/apply_manual_fixes.py`) or leave out. Of the
28 documents that originally landed there unfixed, most of the gap has since been closed
by hand (§2.2a) — 24 recovered and loaded, 4 dropped as unrecoverable or content-free with
reasons recorded in `ingestion/ocr_extraction.json`. Hand-fixing every one of them still
isn't the goal; what changed is that OCR made most of the remainder tractable, where
previously the printed footer itself was the blocker (scanned pages, not extractable text).
...
- **A real near-duplicate ended up in the corpus regardless**: two different DoH GUID URLs
  ...
  moved from hypothetical to actually observed. **Unresolved as of the corpus expansion
  below — still two documents, same code/version/date, one arbitrarily "in force."
  The fix belongs in the data (someone reads both PDFs and decides which is
  authoritative, or the corpus drops one), never in `supersession.py` — a tiebreak rule
  would hide a real corpus ambiguity behind code that looks deterministic.**

### 2.2a OCR recovery batch (2026-09-16) and restoring 3 already-parsed documents

The corpus grew from 37 to 64 documents in one pass, closing most of the
`needs_manual.json` gap described above:

- **24 documents recovered by OCR.** The 28 originally-refused PDFs (no parseable
  printed-metadata footer for either §2.2 parser) were re-extracted with an OCR pass,
  yielding 27 usable records (`ingestion/ocr_extraction.json`) — of which 13 carry a
  genuinely printed `doc_code` (verified verbatim against their own quotes, none
  invented) and the other 11 use pre-approved synthesized codes (checked against every
  existing code for collisions before use). 4 records were dropped, each with a reason
  recorded in `ocr_extraction.json`'s `disposition` field: two had no recoverable
  printed effective date (one HAAD-era document, one 1-page LOINC note), one Arabic
  MOHAP decree was unrecoverable without an Arabic-capable OCR model (the only drop
  worth revisiting, since it's the material that would let the bilingual
  English-query→Arabic-passage claim be demonstrated rather than asserted), and one
  (`DHA/STD/ORGAN-TISSUE-DONATION`) was only caught *after* loading: it rechunks to a
  single chunk with no substantive content (it's a 4-page inspection checklist, not a
  standard), and the suggestion miner independently judged it content-free too. It's
  excluded from `apply_manual_fixes.py` so a from-scratch run won't re-add it, but the
  already-loaded row stays in both local and production (production is insert-only —
  see below — so this one low-value document is a permanent, accepted artifact of the
  copy-up).
- **3 already-parsed official documents restored.** Live `/corpus-stats` showed fewer
  official documents than `parsed_documents.json` contained — these were parsed
  correctly but never loaded into production. Diffed by `sha256` and copied across
  with no re-parsing needed.
- **Two pre-rename HAAD documents** (`PPR/HCP/P0010/07` from 2007, `HAAD/MSSM/SD/1.0`
  from 2016) are in the corpus, ingested under their real printed codes (HAAD was
  DoH's predecessor authority). Correct by the rules, but if DoH has since published
  successors to either, those successors need to be ingested under the *same* code for
  supersession to retire the HAAD-era version automatically — otherwise the old
  document keeps serving as "in force" indefinitely. Worth a check if the corpus grows
  further in this area.
- **Dependency note:** `requirements-ingestion.txt` is uninstallable on Python 3.11
  (`numpy==2.5.2` requires 3.12+, and this project runs 3.11). `docling` and `tiktoken`
  were installed unpinned instead to unblock the recovery batch; the versions that
  actually resolved and worked are `docling==2.127.0` and `tiktoken==0.14.0` — pin
  these in `requirements-ingestion.txt` the next time it's touched, rather than
  re-discovering them.
- **Chunking consistency caveat:** the recovery batch was chunked with whatever Docling
  version `pip install docling` resolved to at the time, which is not guaranteed to be
  the exact version the original 37-document corpus was chunked with. Chunk boundaries
  (and therefore bounding-box provenance) may not be perfectly consistent
  corpus-wide — not observed to cause a visible problem, but worth knowing if chunk
  shapes ever look inconsistent between an old and a new document.

**Seeding a local database from a production dump.** Rather than re-running the full
ingestion pipeline (which re-embeds everything at real OpenAI cost), a local Postgres
can be seeded directly from a `pg_dump` of production:

```
pg_dump --format=custom --no-owner --no-acl --dbname="$PROD_DATABASE_URL" \
  --file=corpus_backup.dump
pg_restore --no-owner --no-acl --clean --if-exists \
  --dbname="postgresql://regulense:regulense_dev_password@localhost:5433/regulense" \
  corpus_backup.dump
```

The dump is `pg_dump`'s custom format (`PGDMP` header) — portable across the minor
Postgres version difference between local Docker Postgres and Supabase's Postgres
17.6. Keep the dump file gitignored (it embeds real embeddings, i.e. paid-for OpenAI
API output) and never commit it.
...
embedding column** — at under a thousand rows (759 live at the time of writing), a
sequential scan with exact cosine distance is faster
...
- **Self-verify, retry once, resume.** The miner whitespace-collapses each anchor and
  ...
  Current corpus state: **263 rows loaded** across the full 64-document corpus. The
  2026-09-16 recovery batch (27 new documents, mined separately from the original 37)
  contributed 24 of those: 21 of the 27 mined successfully, 1 self-skipped as
  content-free, and 5 failed outright — including two documents too large for the
  local mining model's context window even at its largest auto-sized load
  (`DOH/STD/ADHICS` at 308 chunks needing ~70k tokens, `MOHAP/PQR/UNIFIED-QUAL` at 141
  chunks needing ~51k tokens, against a ~43k-token ceiling), so those two currently
  have zero "Continue exploring" suggestions until mined with a larger-context model.
...
Cloud Run deploy jobs that used to sit alongside the test gate targeted
