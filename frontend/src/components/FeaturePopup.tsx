...
// sessionStorage, not localStorage: matches Home/OnboardingWelcome's per-session reset
// (once-ever via localStorage read as "it stopped showing" once dismissed).
export function hasSeenFeaturePopup(): boolean {
  try {
    return sessionStorage.getItem(FEATURE_POPUP_STORAGE_KEY) === SEEN;
  } catch {
    return false;
  }
}
...
export function markFeaturePopupSeen(): void {
  try {
    sessionStorage.setItem(FEATURE_POPUP_STORAGE_KEY, SEEN);
  } catch {
