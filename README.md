...
| Layer | Choice |
|---|---|
| Corpus | Live: 61 official DHA/DoH/MOHAP documents + 3 research papers, 1713 official structure-aware chunks (1757 total including research), 263 mined follow-up questions (`GET /corpus-stats` is the authority — the numbers here are a snapshot). Ingestion funnel: 68 downloaded → 40 parsed automatically → 28 refused for having no parseable printed metadata → 24 recovered by hand (OCR) + 3 restored = 64 documents in the corpus. |
...
| Containers / deploy | Docker + docker-compose (local); Render (self-contained, frontend + API), production |
