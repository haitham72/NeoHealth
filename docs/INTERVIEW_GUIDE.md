...
> Vercel, and every answer carries its document code, version, effective date, and
...
- **Two deployments**: Render builds the frontend into `backend/static` and serves
  same-origin; Vercel serves just the SPA and calls Render cross-origin, so
  `ALLOWED_ORIGINS` must list the exact Vercel URL or every request silently fails.
