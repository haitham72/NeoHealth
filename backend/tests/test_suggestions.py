...
def test_stream_cache_hit_refreshes_suggestions(conn, redis_conn, monkeypatch):
    ...
    list(retrieval.answer_question_stream(conn, question, superseded_filter=False, provider="local"))
    with conn.cursor() as cur:
        cur.execute("DELETE FROM suggested_questions")
    _insert_suggestion(conn, "Second take!", QUERY_VEC, doc_id)

    events = list(retrieval.answer_question_stream(conn, question, superseded_filter=False, provider="local"))
