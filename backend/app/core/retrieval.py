...
from app.core.safety import build_user_message, screen_question
...
# Retries sit strictly INSIDE the existing OpenAI attempt, never around the fallback
# chain -- the two solve different problems and must not fight. A retry is for a blip
# (a momentary 429, a dropped connection, a 503) where the same call a second later
# just works; the NaraRouter fallback is for OpenAI being unavailable for a while.
# So: retry OPENAI_RETRY_ATTEMPTS-1 further times, and if they're all exhausted the
# original `except` fires completely unchanged -- _mark_openai_degraded() then
# NaraRouter, with the cooldown and per-IP soft cap untouched.
#
# Only genuinely transient failures qualify. An auth error or a 400 will never succeed
# on a second attempt; retrying those just adds latency in front of an inevitable
# fallback.
OPENAI_RETRY_ATTEMPTS = 3  # 1 initial + 2 retries
...
def _is_transient_openai_error(exc: BaseException) -> bool:
    ...
    if isinstance(exc, APIStatusError):
        return 500 <= (getattr(exc, "status_code", 0) or 0) < 600
    return False


# Backoff bounded at 1s-4s: with OPENAI_RETRY_ATTEMPTS=3 the actual waits are 1s then
# 2s, so ~3s of added latency worst case -- inside the ~5s budget, which matters
# because the 10s SSE heartbeat (_call_with_heartbeats) has to keep covering the
# connection while this runs. Exposed as a module constant so the latency budget is
# assertable without reaching into tenacity's internals.
OPENAI_RETRY_WAIT = wait_exponential(multiplier=1, min=1, max=4)

# reraise=True so the caller sees the original OpenAI exception, not tenacity's
# RetryError -- the existing `except Exception` blocks would catch either, but a
# wrapped exception would make the logs lie about what actually failed.
_retry_openai = retry(
    retry=retry_if_exception(_is_transient_openai_error),
    stop=stop_after_attempt(OPENAI_RETRY_ATTEMPTS),
    wait=OPENAI_RETRY_WAIT,
    reraise=True,
)
...
def _openai_chat_create(**kwargs):
    """The single retried OpenAI chat call. Split out as its own function purely so
    tenacity has something to wrap -- chat_completion()'s own try/except must keep
    seeing exactly one call that either returns or raises."""
    return client.chat.completions.create(**kwargs)


@_retry_openai
def _openai_embed_create(text: str) -> list[float]:
    """Same treatment for embeddings. Unlike generation there is no second provider to
    fall back to (NaraRouter has no embedding models and the corpus is embedded at
    1536-dim), so a transient failure here fails the whole question -- which makes the
    retry matter more here than anywhere else, not less."""
...
def chat_completion(messages: list[dict], client_ip: str | None = None, stream: bool = False,
    ...
    if not _openai_is_degraded() and not ip_limited:
        try:
            resp = _openai_chat_create(model=CHAT_MODEL, messages=messages, temperature=0, stream=stream, **extra)
...
def _screen_and_route(question: str) -> tuple[str, dict | None]:
    """The shared preamble both entry points run before anything else touches the
    question. Returns (question_to_use, early_result_or_None).

    Order is deliberate: **safety screen first, then the small-talk router.** Every
    input gets screened; nothing bypasses it, and a greeting is not a reason to skip a
    check. Both sit ABOVE the cache probe, so neither a blocked injection attempt nor
    a "hi" costs an embedding, an LLM call, a DB read, or a cache entry -- and neither
    can ever be stored as an answer.

    The returned question is the screened one (PII redacted to typed tokens), and it
    is what the caller must use from here on -- that is what keeps raw identifiers out
    of the embedding call, the cache key, the trace and the persisted chat history.

    Both early results are abstentions carrying their own fixed text, additively
    flagged (`blocked` / `smalltalk`) exactly like the existing `off_topic` flag, so
    app/api/routers/ keeps passing the dict straight through as JSON."""
    screen = screen_question(question)
    if screen["blocked"]:
        return screen["safe_text"], {
            "abstained": True,
            "blocked": True,
    ...
    safe_question = screen["safe_text"]
    smalltalk = conversation.route(safe_question)
    if smalltalk is not None:
        return safe_question, {**smalltalk, "run_id": _current_run_id()}

    return safe_question, None
...
def embed(text: str) -> list[float]:
    # @traceable stays outermost so retries of a transient failure show up as one
    # embed_query span, not as several.
    return _openai_embed_create(text)
...
def _build_messages(
    ...
    # safety.build_user_message, not an inline dict: user text goes in the user role
    # structurally, not by convention (see its docstring). Same bytes as before.
    messages.append(build_user_message(question, f"Sources:\n{context}"))
    return messages
...
def answer_question_stream(
    ...
    question, early_result = _screen_and_route(question)
    if early_result is not None:
        # One event, immediately: there is no pipeline to narrate, and the reasoning
        # trace showing no embedding/search/relevance steps is itself the proof that
        # a greeting cost nothing.
        yield {"step": "done", "result": early_result}
        return
