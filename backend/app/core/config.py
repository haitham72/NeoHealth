...
# Same-origin production (Render serves the API and the built frontend from one
# origin); CORS only needs to cover local dev. ALLOWED_ORIGINS overrides this
# default entirely -- if Render has that env var set to something stale, this
# default is never even reached, so a stale live deployment needs the dashboard
# value updated directly, not just this.
ALLOWED_ORIGINS = os.environ.get(
    "ALLOWED_ORIGINS",
    "http://localhost:5173,http://localhost:8080",
).split(",")

# Optional extra-origin regex, empty by default (same-origin needs none). When set
# via ALLOWED_ORIGIN_REGEX it must stay tightly scoped -- a broad pattern here is
# a cross-origin hole, not a convenience.
