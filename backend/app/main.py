...
from app.api.routers import ask, cache, chats, corpus, cross_check, diff, feedback, health, models, pdf
...
# Production is same-origin (Render serves the built frontend from backend/static),
# so CORS only matters for local dev: docker-compose (nginx on :8080 talking to
# uvicorn on :8000) and Vite dev (:5173). An empty ALLOWED_ORIGIN_REGEX must stay
# None -- an empty-string pattern would match every origin.
app.add_middleware(
    CORSMiddleware,
